...
import { getProjectsList, getFavoriteProjectIds, addProjectFavorite, removeProjectFavorite, type ProjectListItem } from '@/app/actions/projects'
import { getClients } from '@/app/clients/actions'
import { PROJECT_CATEGORIES } from '@/lib/constants'
import { Plus, FolderKanban, Search, Star, SlidersHorizontal } from 'lucide-react'
...
export default function ProjectsPageClient() {
  ...
  const loadProjectRows = useCallback(async () => {
    ...
    if (data) setProjectRows(data as Project[])
  }, [supabase])
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
              ...
              <div className="hidden sm:flex sm:items-center gap-2">
                <Select value={filterClientId} onValueChange={setFilterClientId}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">{t('projects.all_clients')}</SelectItem>
                    {clients.map((c) => (
                ...
                <Select value={filterCategory} onValueChange={setFilterCategory}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">{t('projects.all_categories')}</SelectItem>
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="flex flex-col gap-4 py-2">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t('projects.filter_client')}</label>
                    <Select value={filterClientId} onValueChange={setFilterClientId}>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t('projects.filter_category')}</label>
                    <Select value={filterCategory} onValueChange={setFilterCategory}>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t('projects.all_categories')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">{t('projects.all_categories')}</SelectItem>
                        {PROJECT_CATEGORIES.map((c) => (
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 px-4 bg-card rounded-lg border border-border">
                ...
                <p className="text-muted-foreground text-center">
                  {searchQuery || filterClientId !== 'all' || filterCategory !== 'all'
                ...
                {filtered.map((project) => {
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={(e) =>
                        e.key === 'Enter' && router.push(`/project/${project.id}`)
                      }
                    ...
                    >
                      <button
                        ...
                        aria-label={isFavorite ? t('sidebar.remove_from_favorites') : t('sidebar.add_to_favorites')}
                        title={isFavorite ? t('sidebar.remove_from_favorites') : t('sidebar.add_to_favorites')}
                      >
                        <Star
