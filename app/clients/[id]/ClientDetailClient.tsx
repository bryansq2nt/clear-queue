...
function LinkForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2.5 p-3 rounded-lg bg-muted/50 border border-border">
      {error && <p className="text-sm text-destructive">{error}</p>}
      <div>
        <Label htmlFor="link-url" className="text-sm font-medium text-foreground">{t('clients.link_url_label')}</Label>
        <Input
          id="link-url"
          value={url}
      ...
      <div>
        <Label htmlFor="link-label" className="text-sm font-medium text-foreground">{t('clients.link_label_optional')}</Label>
        <Input
          id="link-label"
          value={label}
      ...
      <div className="flex gap-2">
        <Button type="submit" disabled={saving} size="sm" className="bg-primary text-primary-foreground hover:opacity-90">
          {saving ? t('clients.saving') : t('common.save')}
        </Button>
        <Button type="button" variant="outline" size="sm" onClick={onCancel} className="border-border text-foreground hover:bg-accent">
          {t('common.cancel')}
        </Button>
      </div>
    </form>
            ...
            <section className="bg-card rounded-lg shadow-sm border border-border p-6">
              ...
              <dl className="space-y-3 text-sm">
                <div>
                  ...
                  <dd className="text-foreground font-medium">{client.full_name}</dd>
                </div>
                {client.phone && (
                  <div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            if (!confirm(t('clients.remove_link_confirm'))) return
