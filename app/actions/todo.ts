...
export async function createTodoListAction(
  ...
  if (projectId) {
    await requireCan(user.id, 'todos.create', { type: 'todo', projectId });
  }
...
export async function renameTodoListAction(
  ...
  if (listProjectId) {
    await requireCan(user.id, 'todos.update', {
      type: 'todo',
...
export async function deleteTodoListAction(
  ...
  const listProjectId = (listRow as { project_id?: string } | null)?.project_id;
  if (listProjectId) {
    await requireCan(user.id, 'todos.delete', {
      type: 'todo',
      projectId: listProjectId,
    });
  }
...
export async function createTodoItemAction(
  ...
  const listProjectId = (listRow as { project_id?: string } | null)?.project_id;
  if (listProjectId) {
    await requireCan(user.id, 'todos.create', {
      type: 'todo',
      projectId: listProjectId,
    });
  }
