...
const COLOR_PRESETS = [
  ...
  },
  {
    ...
    secondary: '#064e3b',
    third: '#ecfdf5',
    labelKey: 'preset_emerald',
  },
  {
    primary: '#7c3aed',
    secondary: '#4c1d95',
    third: '#f5f3ff',
    labelKey: 'preset_violet',
  },
  {
    primary: '#dc2626',
    secondary: '#7f1d1d',
    third: '#fef2f2',
    labelKey: 'preset_rose',
  },
  {
    primary: '#2563eb',
    secondary: '#1e3a8a',
...
export default function AppearancePageClient({
  ...
  async function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (result.data) {
      const res = await updatePreferences({
        company_logo_asset_id: result.data.id,
      });
      if (res.error) {
        setError(res.error);
        return;
      ...
      setCompanyLogoUrl(url);
      setPrefs((prev) =>
        prev ? { ...prev, company_logo_asset_id: result.data!.id } : prev
      );
    }
  ...
  async function handleRemoveLogo() {
    ...
    setPrefs((prev) =>
      prev ? { ...prev, company_logo_asset_id: null } : prev
    );
  }
  ...
  async function handleCoverChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (result.data) {
      const res = await updatePreferences({
        cover_image_asset_id: result.data.id,
      });
      if (res.error) {
        setError(res.error);
        return;
      ...
      setCoverImageUrl(url);
      setPrefs((prev) =>
        prev ? { ...prev, cover_image_asset_id: result.data!.id } : prev
      );
    }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto w-full">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-foreground mb-4">
            {t('settings.brand_colors')}
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
            {t('settings.preset_swatches')}
          </p>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                <input
                  type="color"
                  value={
                    isValidHex(form.third_color) ? form.third_color : '#F4F7FB'
                  }
                  onChange={(e) =>
                    setForm((f) => ({ ...f, third_color: e.target.value }))
                  }
                  className="w-10 h-10 rounded border border-border cursor-pointer bg-background"
                />
                <Input
