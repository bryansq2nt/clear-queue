-- =============================================================================
-- Fix: accept_invite_atomic must copy granted_actions from project_invite_roles
-- to user_project_action_grants when an invite was created with a custom role.
--
-- Root cause: the previous version read effective_role_name and allowed_modules
-- from project_invite_roles but silently dropped granted_actions. After accepting,
-- the member's RBAC resolver fell back to role-based grants (project_viewer →
-- tasks.read.project) instead of the custom per-action grants intended by the
-- invite creator.
-- =============================================================================
...
BEGIN
  ...
  -- 5b. Apply custom action grants from the invite role.
  --     These override role-based grants and determine exact permissions
  --     (e.g. tasks.read.own vs tasks.read.project).
...
COMMENT ON FUNCTION public.accept_invite_atomic(TEXT, UUID) IS
  'Accepts a project invite for the given user. Copies granted_actions from the invite role to user_project_action_grants so custom per-action permissions (e.g. tasks.read.own) are applied immediately.';
